...
export function SharedEvents({ onAccept, onDecline, onAddToCalendar }: SharedEventsProps) {
      ...
      <CardHeader>
        <div className="flex items-center justify-between">
          <div>
            ...
            <CardDescription>Events shared with you by other students</CardDescription>
          </div>
          <Badge variant="outline" className="ml-auto">
            {sampleSharedEvents.filter((e) => e.status === "pending").length} pending
          </Badge>
        </div>
      </CardHeader>
      <CardContent>
                    ...
                    <div key={event.id} className="rounded-lg border p-4">
                      <div className="flex items-start justify-between">
                        <div>
                          <div className="flex items-center gap-2">
                            ...
                            <Badge
                              className="ml-2"
                              style={{
                                backgroundColor: EVENT_TYPES[event.type]?.color.replace("bg-", ""),
                              }}
                            >
                        ...
                        <div className="flex items-center">
                          <Avatar className="h-8 w-8">
                            <AvatarImage src={event.sharedBy.image || "/placeholder.svg"} alt={event.sharedBy.name} />
                            <AvatarFallback>{event.sharedBy.name.charAt(0)}</AvatarFallback>
                          </Avatar>
                        </div>
                      </div>
                      ...
                      <div className="mt-4 flex items-center justify-between">
                        <div className="text-xs text-muted-foreground">
                          {event.allowEdit && <span className="italic">You can edit this event</span>}
                        </div>
                        <div className="flex gap-2">
                          {event.status === "pending" ? (
                            ...
                            <Button size="sm" variant="outline" onClick={() => onAddToCalendar(event)}>
                              View in Calendar
                            </Button>
                          ) : (
                            <Button
