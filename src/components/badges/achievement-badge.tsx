...
export function AchievementBadge({ badge, size = "md", showDetails = false, className }: AchievementBadgeProps) {
  const isEarned = !!badge.earnedAt
  const isInProgress = !isEarned && badge.progress && badge.progress.current > 0
    ...
    <div className={cn("flex flex-col items-center", className)}>
      <div
        className={cn(
          ...
          isEarned ? rarityBorders[badge.rarity] : "border-gray-300 dark:border-gray-700",
          sizeClasses[size],
        )}
      >
        <div
          className={cn(
            ...
            isEarned ? rarityColors[badge.rarity] : "bg-gray-200 dark:bg-gray-800",
          )}
        />
      ...
      {showDetails && (
        <div className="mt-2 text-center">
          <h4 className={cn("font-medium", detailsSize[size])}>
            {badge.name}
            {isEarned && (
              <Badge variant="outline" className={cn("ml-2", size === "sm" ? "text-[10px] px-1" : "")}>
                {badge.rarity}
              </Badge>
            )}
          </h4>
          ...
          {isInProgress && badge.progress && (
            <div className="mt-1 w-full">
              ...
              <Progress value={(badge.progress.current / badge.progress.required) * 100} className="h-1" />
            </div>
          )}

          {!isEarned && !isInProgress && (
            <p className={cn("text-muted-foreground", size === "sm" ? "text-[10px]" : "text-xs")}>Locked</p>
          )}
        </div>
      )}
    </div>
