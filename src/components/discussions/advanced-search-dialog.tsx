...
export function AdvancedSearchDialog() {
  ...
  const onSubmit = (data: FormValues) => {
    ...
    if (data.status && data.status.length) params.append("status", data.status.join(","))
    if (data.courses && data.courses.length) params.append("courses", data.courses.join(","))
    if (data.participants && data.participants.length) params.append("participants", data.participants.join(","))
    if (data.dateRange?.from) params.append("from", data.dateRange.from.toISOString())
    if (data.dateRange?.to) params.append("to", data.dateRange.to.toISOString())
    if (data.messageCount?.min) params.append("minMessages", data.messageCount.min)
    if (data.messageCount?.max) params.append("maxMessages", data.messageCount.max)
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="sm:max-w-[700px]">
        <DialogHeader>
          ...
          <DialogDescription>
            Use the form below to search for specific discussions across the platform.
          </DialogDescription>
        </DialogHeader>
              ...
              <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        ...
                        <FormControl>
                          <Input placeholder="Search in title and content..." {...field} />
                        </FormControl>
                        <FormDescription>Search in discussion titles and messages</FormDescription>
                      </FormItem>
                    )}
                  />
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem className="flex flex-col">
                      ...
                      <FormDescription>Filter discussions by creation date</FormDescription>
                    </FormItem>
                  )}
                />
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                                  ...
                                  {courses.map((course) => (
                                    <CommandItem
                                      ...
                                      onSelect={() => {
                                        ...
                                        const newValues = currentValues.includes(course.id)
                                    ...
                                    >
                                      <Check
                                        className={`mr-2 h-4 w-4 ${
                                          field.value?.includes(course.id) ? "opacity-100" : "opacity-0"
                                        }`}
                                      />
                                      {course.title}
                                    </CommandItem>
                      ...
                      <FormDescription>Filter discussions by specific courses</FormDescription>
                      {field.value?.length ? (
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <FormDescription>Filter by students or instructors involved</FormDescription>
                      {field.value?.length ? (
                        <div className="flex flex-wrap gap-1 mt-2">
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <FormDescription>Filter by number of messages in the discussion</FormDescription>
                    </FormItem>
                  )}
                />
                    ...
                    render={({ field }) => (
                      <FormItem className="flex flex-row items-start space-x-3 space-y-0">
                        ...
                        <div className="space-y-1 leading-none">
                          ...
                          <FormDescription>Show only private discussions</FormDescription>
                        </div>
                      </FormItem>
                    )}
                  />
                    ...
                    render={({ field }) => (
                      <FormItem className="flex flex-row items-start space-x-3 space-y-0">
                        ...
                        <div className="space-y-1 leading-none">
                          ...
                          <FormDescription>With files or images</FormDescription>
                        </div>
                      </FormItem>
                    )}
                  />
                    ...
                    render={({ field }) => (
                      <FormItem className="flex flex-row items-start space-x-3 space-y-0">
                        ...
                        <div className="space-y-1 leading-none">
                          ...
                          <FormDescription>Contains flagged messages</FormDescription>
                        </div>
                      </FormItem>
                    )}
                  />
                ...
                <DialogFooter className="gap-2 sm:gap-0">
                  <div className="flex items-center gap-2">
                    ...
                    <Button type="button" variant="secondary" onClick={saveCurrentSearch}>
                      Save Search
                    </Button>
                  </div>
          ...
          <TabsContent value="saved" className="mt-4">
            <div className="space-y-4">
              <p className="text-sm text-muted-foreground">
                Select a saved search to quickly filter discussions using predefined criteria.
              </p>
                ...
                {savedSearches.map((search) => (
                  <div
                    ...
                    className={`p-3 rounded-md border cursor-pointer transition-colors ${
                      selectedPreset === search.id ? "bg-primary/10 border-primary" : "hover:bg-muted"
                    }`}
              ...
              {savedSearches.length === 0 && (
                <div className="text-center py-8 text-muted-foreground">
                  ...
                  <p className="text-sm mt-1">Create and save search criteria from the Search Criteria tab.</p>
                </div>
              )}
              ...
              <DialogFooter>
                ...
                <Button
                  ...
                  onClick={() => {
                    if (selectedPreset) {
                      const preset = savedSearches.find((s) => s.id === selectedPreset)
                      if (preset) {
                        form.reset(preset.criteria as any)
